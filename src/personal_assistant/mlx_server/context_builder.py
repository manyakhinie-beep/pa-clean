...
class ContextAssembler:
    ...
    def _make_system_prompt(
        ...
        # Жёсткий язык вывода — пользователь явно потребовал везде во всех
        # дефолтных промптах: «отвечать только на русском языке».  Ставим
        # инструкцию сразу после identity, чтобы она имела приоритет над
        # любой персонализацией / souls / тулами, которые могут влиять на стиль.
        parts.append("Отвечай только на русском языке.")
        if ai_style:
